...
struct MutInfoDistance {
  ...
  double operator()( const size_t varA, const size_t varB ) { return compute(varA,varB); }
  double compute( const size_t varA, const size_t varB );
  ...
  virtual void invalidCache() {
    distCache.clear();
    entropyMap.clear();
    entropyMap.resize(positions.size(), -1.0);
...
template<class DM>
double MutInfoDistance<DM>::compute( size_t varA, size_t varB ) {
