...
struct Entropy
{
  ...
  double operator()(const VecT& xVec) { return compute(xVec); }
  
 protected:
  template<typename VecT>
  double operator()(const VecT& xVec, utility::Int2Type<EMP>);
...
struct JointEntropy
{
  ...
  double operator()( const VecXT& xVec, const VecYT& yVec ) { return compute(xVec, yVec); }

 protected:  
  template<typename VecXT, typename VecYT>
  double operator()( const VecXT& xVec, const VecYT& yVec, utility::Int2Type<EMP>); 
};
...
template<typename VecT>
double Entropy<EstimationMethodT>::compute( const VecT& xVec, utility::Int2Type<EMP>) {
  std::map<unsigned, unsigned> xCountMap; 
  for ( size_t i = 0; i < xVec.size(); ++i ) {
    updateCountMap( xCountMap, xVec.at(i) );    
  }

  double vecLen = 1.0*xVec.size();
  double xLogSum = accumulate( xCountMap.begin(), xCountMap.end(), 0.0, sumLogCount<unsigned> );
  double enp = log(vecLen) - 1/vecLen*xLogSum;
...
template<int EstimationMethodT>
template<typename XVec, typename YVec>
double JointEntropy<EstimationMethodT>::operator()( const VecXT& xVec, const VecYT& yVec, utility::Int2Type<EMP>) {
  typedef std::pair<unsigned, unsigned> IntPair;
  std::map<IntPair, unsigned> jointCountMap;  
  double vecLen = 0.0;
  for ( size_t i = 0; i < xVec.size(); ++i )  {
    const IntPair jointKey = std::make_pair( xVec.at(i), yVec.at(i) ); 
