...
do
    CHR_DIR=$(basename $D)    
    echo "${EXE}/${exe} -i ${D}/casecontrol_filtered_data.csv -l ${D}/casecontrol_filtered_label.csv -o ${OUT_PATH}/CAST/${CHR_DIR}_${PARAMS}.txt ${KP} -e ${EPS} -s ${SIMI} -x ${MAX_DIST} -v 1 "
    eval "${EXE}/${exe} -i ${D}/casecontrol_filtered_data.csv -l ${D}/casecontrol_filtered_label.csv -o ${OUT_PATH}/CAST/${CHR_DIR}_${PARAMS}.txt ${KP} -e ${EPS} -s ${SIMI} -x ${MAX_DIST} -v 1"
done
