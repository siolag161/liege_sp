...
if  [[ -z $BED_PATH ]] || [[ -z $OUT_PATH ]] || [[ -z $PLINK ]] 
then
...
if [ ! -f "${bed_path_without_extension}.bed" ]; then
  echo "${bed_path_without_extension}.bed file not found" 
  echo
  usage
  exit
fi
...
out_path="${out_path}/${bed_filename_without_extension}_${PERMU}"

command="${PLINK}/plink --noweb --bfile  ${ped_path} --out ${out_path} --assoc --mperm ${PERMU}"
